...
Add_file <- "C:/Users/minyy/Documents/Kriging/data/A1_max_inun_rr0.75.asc" # Replace with path to file in the data folder
...
if (!file.exists("dist_map_A1_0.75.RData")) {dist_map_A1_0.75 <- distance(filtered_clust_on_road_2) 
save(dist_map_A1_0.75, file = "dist_map_A1_0.75.RData")} else {load("dist_map_A1_0.75.RData")}
...
user$DistanceToFlood_2 <- raster::extract(dist_map_A1_0.75, user)
...
v1 <- variogram(g1, cutoff = 700, width = 42)
fit1 <- fit.lmc(v1, g1)
plot(v1, model = fit1)
...
risk2_thresh <- calc(risk2_masked, fun = function(x) ifelse(x >= 0.79, 1, 0))
